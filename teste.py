...
def crop_image_circle():
    ...
    for idx, circle in enumerate(circle_data):
        ...
        # Ajusta os limites para garantir que estejam dentro da imagem
        x_start, y_start = max(x_start, 0), max(y_start, 0)
        x_end, y_end = min(x_end, image.shape[1]), min(y_end, image.shape[0])
        
        # Recorta a imagem para a área do retângulo
        cropped_result = image[y_start:y_end, x_start:x_end]
        
        # Cria uma máscara circular
        mask = np.zeros((cropped_result.shape[0], cropped_result.shape[1]), dtype=np.uint8)
        cv.circle(mask, (center[0] - x_start, center[1] - y_start), radius, (255), thickness=-1)
        
        # Aplica a máscara circular
        circular_cropped_result = cv.bitwise_and(cropped_result, cropped_result, mask=mask)
        
        # Define o caminho para salvar a imagem recortada
        output_path = os.path.join(os.getcwd(), f'cropped_circle_image_{idx}.png')
        cv.imwrite(output_path, circular_cropped_result)
            ...
            with dpg.group():
                with dpg.child_window(height=380, border=False):
                    with dpg.group():
                        ...
                        dpg.add_button(label="RECORTAR IMAGEM", width=150, height=30, callback=crop_image_circle)
